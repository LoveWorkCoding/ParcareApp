...
public class Parcare implements Serializable {
  ...
  public void scoate(Masina m) {
    if (masini.contains(m)) {
      ...
      for (Istoric i : istoric) {
        if (i.getMasina().equals(m)) {
          istoric.add(new Istoric(m, i.getTimpIntrare(), dataIesire));
          istoric.remove(i);
          break;
        }
      }
  ...
  public double calculTaxa(Masina m) {
    double taxaFinala = taxa;
    for (Istoric i : istoric) {
      if (i.getMasina().equals(m)) {
        this.timp = t.timpScurs(i.getTimpIntrare(), i.getTimpIesire());
        if (timp.getMinute() > 0)
  ...
  public String getData(String text) {
    ...
    String istoricMasina = "";
    for (Istoric i : istoric) {
      if (i.getMasina().getNume().equals(nume[0]) && i.getMasina().getNumarInmatriculare().toString().equals(nume[1])) {
        if (!istoricMasina.equals("")) {
          istoricMasina += "\n Data de intrare: " + i.getTimpIntrare() + "\n Data de iesire: " + i.getTimpIesire() + "\n";
        } else {
          istoricMasina += i.toString();
        }
      }
    }
    return istoricMasina;
  }
